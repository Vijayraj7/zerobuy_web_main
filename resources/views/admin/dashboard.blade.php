...
@section('content')
    ...
    @php
        ...
        $showWalletSection = ($generaleSetting?->business_based_on ?? null) === 'commission';
    @endphp
        ...
        <div class="card-body">
            <div class="cardTitleBox d-flex align-items-center justify-content-between flex-wrap gap-2">
                <h5 class="card-title chartTitle mb-0">
                    {{ __('Latest Created Shops & Customers') }}
                </h5>
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    @endphp
                    @foreach ($createdFilters as $filterKey => $filterLabel)
                        <a href="{{ route('admin.dashboard.index', ['created_filter' => $filterKey]) }}"
                            class="statisticsBtn {{ $createdFilter == $filterKey ? 'active' : '' }}">
                            {{ $filterLabel }}
                        </a>
                    @endforeach
                </div>
            ...
            <div class="row g-3 mt-1">
                <div class="col-xxl-6">
                    <div class="border rounded p-3 h-100">
                        <h6 class="mb-3">{{ __('Last Created Shops') }}</h6>
                        <div class="d-flex flex-column gap-1">
                            @forelse ($latestCreatedShops as $recentShop)
                                <a href="{{ route('admin.shop.show', $recentShop->id) }}" class="customer-section">
                                    <div class="customer-details">
                                        <div class="customer-image">
                                            <img src="{{ $recentShop->logo }}" alt="logo" loading="lazy" />
                                        </div>
                                        <div class="customer-about">
                                            <p class="name text-dark mb-0">
                                                {{ Str::limit($recentShop->name, 30, '...') }}
                                            </p>
                                            <p class="order mb-0">
                                                {{ __('Owner') }}:
                                                {{ Str::limit($recentShop->user?->name ?? __('N/A'), 20, '...') }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="border text-black px-2 py-1 flex-shrink-0"
                                        style="font-size: 13px; border-radius: 25px;">
                                        <div>{{ $recentShop->created_at->format('d M, Y') }}</div>
                                    </div>
                                </a>
                            @empty
                                <p class="text-muted mb-0">{{ __('No shops found in selected period') }}</p>
                            @endforelse
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xxl-6">
                    <div class="border rounded p-3 h-100">
                        <h6 class="mb-3">{{ __('Last Created Customers') }}</h6>
                        <div class="d-flex flex-column gap-1">
                            @forelse ($latestCreatedCustomers as $recentCustomer)
                                <a href="{{ route('admin.customer.show', $recentCustomer->user_id) }}"
                                    class="customer-section">
                                    <div class="customer-details">
                                        <div class="customer-image">
                                            <img src="{{ $recentCustomer->user?->thumbnail ?? asset('default/profile.jpg') }}"
                                                alt="customer" loading="lazy" />
                                        </div>
                                        <div class="customer-about">
                                            <p class="name text-dark mb-0">
                                                {{ Str::limit($recentCustomer->user?->name ?? __('N/A'), 30, '...') }}
                                            </p>
                                            <p class="order mb-0">
                                                {{ Str::limit($recentCustomer->user?->email ?? __('No email'), 28, '...') }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="border text-black px-2 py-1 flex-shrink-0"
                                        style="font-size: 13px; border-radius: 25px;">
                                        <div>{{ __('Orders') }}: {{ $recentCustomer->orders_count }}</div>
                                    </div>
                                </a>
                            @empty
                                <p class="text-muted mb-0">{{ __('No customers found in selected period') }}</p>
                            @endforelse
                        </div>
                    </div>
                </div>
